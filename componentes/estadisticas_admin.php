...
$stats = [
    ...
    'ingresosMes' => $hasPagos ? admin_scalar_float($conn, "SELECT COALESCE(SUM(GREATEST(monto_pagado + credito_aplicado - descuento_autorizado, 0)), 0) FROM contabilidad_registro_pagos WHERE estado_financiero NOT IN ('cortesia', 'exento') AND updated_at >= DATE_FORMAT(CURDATE(), '%Y-%m-01')") : 0.0,
    'ingresosTotales' => $hasPagos ? admin_scalar_float($conn, "SELECT COALESCE(SUM(GREATEST(monto_pagado + credito_aplicado - descuento_autorizado, 0)), 0) FROM contabilidad_registro_pagos WHERE estado_financiero NOT IN ('cortesia', 'exento')") : 0.0,
